      ...
      <div className="relative mx-auto flex h-full w-full max-w-[440px] flex-col overflow-hidden bg-[#0b1020] text-white">
        ...
        {screen === "LOGIN" && (
          <section className="relative flex h-full flex-col items-center justify-center overflow-hidden px-6">
            <div className="cpp-road absolute inset-0 opacity-60" aria-hidden />
            <div className="absolute inset-0 bg-gradient-to-b from-[#0b1020]/80 to-[#0b1020]/95" aria-hidden />
            <form
              onSubmit={(e) => {
                e.preventDefault()
              ...
              className="relative z-10 w-full max-w-sm rounded-3xl border border-white/25 bg-white/10 p-6 shadow-2xl backdrop-blur-xl"
            >
              <h2 className="mb-1 text-center text-2xl font-black">환영해요! 👋</h2>
              <p className="mb-6 text-center text-sm text-white/70">닉네임을 정하고 팡팡을 시작해요</p>
              <label className="mb-2 block text-sm font-bold text-white/80" htmlFor="nick">
                닉네임
              </label>
              <input
                id="nick"
                ...
                maxLength={12}
                placeholder="예: 계산왕팡이"
                className="mb-5 w-full rounded-xl border border-white/25 bg-white/15 px-4 py-3 text-lg font-bold text-white placeholder:text-white/40 outline-none focus:border-yellow-300"
              />
              <button
                type="submit"
            ...
            <div className="relative min-h-0 flex-1 overflow-hidden">
              ...
              {isExample && (
                <button
                  ...
                  className="absolute right-3 top-2 z-20 rounded-full bg-white/20 px-3 py-1.5 text-xs font-black text-white ring-1 ring-white/30 active:scale-95"
                >
                  Skip ⏭
                </button>
              )}
              ...
              <div className="max-h-[60vh] overflow-y-auto px-5 pb-8 cpp-no-scrollbar">
                {clearedLevels.length === 0 ? (
                  <p className="py-10 text-center text-sm text-white/60">
                    아직 통과한 레벨이 없어요.
                    <br />
                    레벨을 클리어하면 팡이 팩을 모을 수 있어요! 🐣
                  </p>
                ) : (
                  <div className="space-y-4">
                    <p className="text-xs text-white/60">통과한 레벨의 팡이 10종 팩을 저장할 수 있어요.</p>
                    {clearedLevels.filter((lv) => lv > 0).map((lv) => (
                      <div key={lv} className="rounded-2xl bg-white/5 p-3 ring-1 ring-white/10">
                        <div className="mb-2 flex items-center justify-between">
                          ...
                          <button
                            type="button"
                            onClick={() => downloadPang(lv)}
                            className="rounded-full bg-gradient-to-b from-pink-400 to-pink-500 px-3 py-1.5 text-xs font-black text-white shadow-[0_3px_0_#be185d] active:translate-y-0.5"
                          >
                            ⬇ 다운로드
                          </button>
                        </div>
                        <div className="flex gap-2 overflow-x-auto cpp-no-scrollbar">
                          {Array.from({ length: 10 }, (_, i) => (
                            <GameImage
                              key={i}
                              src={`/assets/${lv}-${i + 1}.png`}
                              alt={`Lv.${lv} 팡이 ${i + 1}`}
                              fallback={
                                <img src="/assets/pangi.png" alt="" className="h-full w-full object-contain" draggable={false} />
                              }
                              className="h-14 w-14 flex-none rounded-xl bg-white/10 object-contain p-1"
